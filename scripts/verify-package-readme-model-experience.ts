...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/auth/auth': { kind: 'none', reason: 'The authentication seam declares an abstract service contract and registers no prompt, tool schema, or model-visible content; gate and UI consumers own any model-facing effect.' },
  'packages/auth/authn-local': { kind: 'none', reason: 'The local authentication provider serves same-process callers over ctx.authn; nothing it stores or returns reaches a model request.' },
  'packages/auth/auth-gate': { kind: 'none', reason: 'The HTTP gate sits on the browser-facing carrier; its pages, redirects, and cookie contract never reach a model request.' },
  'packages/bundle/auth-bundle': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; the rows it inserts own their behavior, none of it model-facing.' },
