...
flowchart LR
  pkg_auth["auth"]
  svc_authn["ctx.authn<br/>Authentication seam"]
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.authn` | `seam` | [`auth`](../packages/auth/auth) | [`authn-local`](../packages/auth/authn-local) | - | - | 该 seam 将 bearer token 解析为账户并负责口令登录；强制执行由 auth-gate 的页面/重定向与 client-connection 的 /api 闸门承担，会话属主绑定仍待后续。 |
